...
Please refer to `PCI Address map <./AWS-Fpga-Pcie-Memory-Map.html>`__ for
...
`AWS_CLK_GEN <./AWS-CLK-GEN-spec.html>`__ IP that supports multiple
...
Please refer to the
`Clock Recipes User Guide <./Clock-Recipes-User-Guide.html>`__ for
...
- SDA AXI-L: Associated with MgmtPF, BAR4. If the developer is using AWS
  ...
  `AWS_CLK_GEN IP <./AWS-CLK-GEN-spec.html>`__ for CL clock management.
...
Please see the `Shell Floorplan <./shell-floorplan.html>`__ for additional
