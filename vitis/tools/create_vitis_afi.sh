...
if ! source $AWS_FPGA_REPO_DIR/shared/bin/set_common_functions.sh; then
    echo "Couldn't set up common functions from withn sub-shell!"
    exit 1
fi

if [[ -z $AWS_FPGA_REPO_DIR ]]; then
    err_msg "\$AWS_FPGA_REPO_DIR not set! Please source vitis_setup.sh from this repository's root directory"
    exit 1
fi
...
function usage {
    echo "USAGE: $script_name [-h|-help] [-s3_bucket=<bucket_name>] [-s3_dcp_key=<S3_folder_of_dcp>] [-s3_logs_key=<S3_folder_of_logs>] [-xclbin=<xclbin_filename>] [-o=<awsxclbin_filename>] [-aws_profile_name=<aws_profile_name>]"
}
...
function help {
    ...
    info_msg "  (*) Ready to create an AFI and test on F2.  Your kernel has been validated using HW Emulation."
    ...
    info_msg "  (2) Generate a Manifest file for AFI creation that sets the clocks based on your achieved target freq from your build"
    info_msg "  (3) Prepare tar file for AFI creation process"
    info_msg "  (4) Call aws ec2 create-fpga-image"
    info_msg "  (5) Create new XCLBIN (called AWSXCLBIN) using Metadata and AGFI-ID"
    echo " "
    usage
}
...
if [ -z "$RELEASE_VER" ]; then
    err_msg "Env variable RELEASE_VER not set, did you source vitis_setup.sh?"
    exit 1
fi
...
if [ "$awsxclbin" != "$stripped_xclbin" ]; then
...
if [[ -e "${timestamp}_SH_CL_routed.dcp" ]]; then
...
if [[ -e "${timestamp}_build.json" ]]; then
...
if [[ -e "${timestamp}_clocks.json" ]]; then
...
read -a clock_main_a0 <<<"$(echo $(grep -B 1 SYSTEM ${timestamp}_clocks.json | grep -o -e '[0-9]*'))"
clock_main_a0=$(echo ${clock_main_a0[0]})
...
if [[ "$vendor" != "xilinx" && "$board_id" != "aws-vu47p-f2" && "$plat_name" != "xdma-shell-v10162423" && "$major" != "202410" && "$minor" != "1" ]]; then
    err_msg "Platform ${vendor}_${board_id}_${plat_name}_${major}_${minor} used to create xclbin is not correct, you should be using xilinx_aws-vu47p-f2_xdma-shell-v10162423_202410_1"
    exit
fi
...
if ! aws s3 ${profile_text} cp ${timestamp}_Developer_Vitis_Kernel.tar s3://${s3_bucket}/${s3_dcps}/; then
    err_msg "Couldn't copy files to s3://${s3_bucket}/${s3_dcps}/"
    exit 1
fi

if ! aws ec2 ${profile_text} create-fpga-image --name ${stripped_xclbin} --description ${stripped_xclbin} --input-storage-location Bucket=${s3_bucket},Key=${s3_dcps}/${timestamp}_Developer_Vitis_Kernel.tar ${log_storage_text} >${timestamp}_afi_id.txt; then
    err_msg "Failed to create AFI from .xclbin file!"
    exit 1
fi
