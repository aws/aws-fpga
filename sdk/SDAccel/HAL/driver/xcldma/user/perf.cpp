...
namespace xclxdma {
  ...
  bool XDMAShim::isDSAVersion(unsigned majorVersion, unsigned minorVersion, bool onlyThisVersion) {
  ...
  unsigned XDMAShim::getBankCount() {
    return mDeviceInfo.mDDRBankCount;
  }

  void XDMAShim::xclSetOclRegionProfilingNumberSlots(uint32_t numSlots) {
  ...
  uint64_t XDMAShim::getHostTraceTimeNsec() {
    struct timespec now;
    int err;
  ...
  uint64_t XDMAShim::getPerfMonBaseAddress(xclPerfMonType type) {
  ...
  uint64_t XDMAShim::getPerfMonFifoBaseAddress(xclPerfMonType type, uint32_t fifonum) {
    if (type == XCL_PERF_MON_MEMORY) {
      // Only one FIFO in >= v2.2
      if (isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false))
        return PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_TRACE_OFFSET_0;

      if (fifonum == 0) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_TRACE_OFFSET_0);
      if (fifonum == 1) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_TRACE_OFFSET_1);
      if (fifonum == 2) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_TRACE_OFFSET_2);
      return 0;
    }
  ...
  uint64_t XDMAShim::getPerfMonFifoReadBaseAddress(xclPerfMonType type, uint32_t fifonum) {
    if (type == XCL_PERF_MON_MEMORY) {
      // Use AXI-MM to access trace FIFO
      // NOTE: requires compatible change in base platform
      if (isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false))
  ...
  uint32_t XDMAShim::getPerfMonNumberFifos(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY)
  ...
  uint32_t XDMAShim::getPerfMonNumberSlots(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY) {
  ...
  uint32_t XDMAShim::getPerfMonNumberSamples(xclPerfMonType type) {
  ...
  uint32_t XDMAShim::getPerfMonByteScaleFactor(xclPerfMonType type) {
    // NOTE: In the NWL DMA base platform, the APM slot data was only 32 bits
    // while the MIG interface was 512 bits
    //if (type == XCL_PERF_MON_MEMORY && isDSAVersion(1, 0, true))
    //  return 16;
    return 1;
  }

  uint8_t XDMAShim::getPerfMonShowIDS(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY) {
  ...
  uint8_t XDMAShim::getPerfMonShowLEN(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY) {
      if (getBankCount() > 1)
  ...
  uint32_t XDMAShim::getPerfMonSlotStartBit(xclPerfMonType type, uint32_t slotnum) {
  ...
  uint32_t XDMAShim::getPerfMonSlotDataWidth(xclPerfMonType type, uint32_t slotnum) {
  ...
  double XDMAShim::xclGetDeviceClockFreqMHz() {
  ...
  double XDMAShim::xclGetReadMaxBandwidthMBps() {
    return 9600.0;
  }

  // Get the maximum bandwidth for host writes to the device (in MB/sec)
  // NOTE: for now, set to: (256/8 bytes) * 300 MHz = 9600 MBps
  double XDMAShim::xclGetWriteMaxBandwidthMBps() {
    return 9600.0;
  }

  // Convert binary string to decimal
  uint32_t XDMAShim::bin2dec(std::string str, int start, int number) {
    return bin2dec(str.c_str(), start, number);
  }

  // Convert binary char * to decimal
  uint32_t XDMAShim::bin2dec(const char* ptr, int start, int number) {
  ...
  std::string XDMAShim::dec2bin(uint32_t n) {
  ...
  std::string XDMAShim::dec2bin(uint32_t n, unsigned bits) {
    char result[bits + 1];
    unsigned index = bits;
    result[index] = '\0';
  ...
  size_t XDMAShim::resetFifos(xclPerfMonType type) {
  ...
  size_t XDMAShim::xclPerfMonStartCounters(xclPerfMonType type) {
    if (mLogStream.is_open()) {
  ...
  size_t XDMAShim::xclPerfMonStopCounters(xclPerfMonType type) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", "
  ...
  size_t XDMAShim::xclPerfMonReadCounters(xclPerfMonType type, xclCounterResults& counterResults) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
      << ", " << type << ", " << &counterResults
  ...
  size_t XDMAShim::xclPerfMonClockTraining(xclPerfMonType type) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", "
  ...
  size_t XDMAShim::xclPerfMonStartTrace(xclPerfMonType type, uint32_t startTrigger) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
      << ", " << type << ", " << startTrigger
  ...
  size_t XDMAShim::xclPerfMonStopTrace(xclPerfMonType type) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", "
  ...
  uint32_t XDMAShim::xclPerfMonGetTraceCount(xclPerfMonType type) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
      << ", " << type << std::endl;
    }
  ...
  size_t XDMAShim::xclPerfMonReadTrace(xclPerfMonType type, xclTraceResultsVector& traceVector) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
      << ", " << type << ", " << &traceVector
