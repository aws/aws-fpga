...
static int
write_worker_function(void *data)
{
	...
	while (!test_bit(XDMA_WORKER_STOP_REQUEST_BIT, &command_queue->thread_status)) {
		if (command_queue->head != command_queue->tail) {
			int ret = xdma_xfer_submit(command_queue->channel_handle, DMA_TO_DEVICE,
									   queue[command_queue->head].target_addr,
									   &queue[command_queue->head].sgt, true, XDMA_TIMEOUT_IN_MSEC);
			if (ret < 0) {
				pr_err("Thread failed during transaction with address 0x%llx and size %u\n",
					   queue[command_queue->head].target_addr,
					   sg_dma_len(queue[command_queue->head].sgt.sgl));
				test_and_set_bit(XDMA_WORKER_STOPPED_ON_TIMEOUT_BIT, &command_queue->thread_status);
				do_exit(-1);
			}

			queue[command_queue->head].completed = 1;

			command_queue->head = EDMA_RING_IDX_NEXT(command_queue->head,edma_queue_depth);
	...
	set_bit(XDMA_WORKER_STOPPED_ON_REQUEST_BIT, &command_queue->thread_status);

	return 0;
}

static int
edma_xdma_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	int ret = 0;
	...
	for (i = 0; i < number_of_xdma_channels; i++) {
		...
		command_queue[i].queue = (command_t*)kzalloc(edma_queue_depth * sizeof(command_t),
											 GFP_KERNEL);
...
static void
edma_xdma_remove(struct pci_dev *pdev)
{
	struct device *dev = &pdev->dev;
...
int
edma_backend_register_isr(struct pci_dev *pdev, u32 event_number, irq_handler_t handler, \
						  const char* name, void* drv)
{
...
int
edma_backend_enable_isr(struct pci_dev *pdev, u32 event_number)
{
	return xdma_user_isr_enable(pdev,BIT(event_number));
}

int
edma_backend_disable_isr(struct pci_dev *pdev, u32 event_number)
{
...
int
edma_backend_submit_m2s_request(u64* buffer, u32 size, void *q_handle, u64 target_addr)
{
	...
	/* 32-bit is atomic */
	command_queue->tail = EDMA_RING_IDX_NEXT(command_queue->tail, edma_queue_depth);
	...
	while ((wake_up_process(command_queue->worker_thread) == 0)
		   && test_bit(XDMA_WORKER_SLEEPING_STATUS_BIT, &command_queue->thread_status))
...
/* TODO: inline */
int
edma_backend_get_completed_m2s_request(u64** buffer, u32* size, void *q_handle)
{
	command_queue_t* command_queue = (command_queue_t*)q_handle;
	command_t* queue = command_queue->queue;
...
int
edma_backend_submit_s2m_request(u64* buffer, u32 size, void *q_handle, u64 target_addr)
{
...
int
edma_backend_get_completed_s2m_request(u64** buffer, u32* size, void *q_handle)
{
	c2h_handle_t* c2h_handle = (c2h_handle_t*)q_handle;
...
int
edma_backend_m2s_dma_action(void *tx_dma_q)
{
	(void)tx_dma_q;
...
int
edma_backend_start(void *q_handle, int minor)
{
	command_queue_t* command_queue = (command_queue_t*)q_handle;
...
int
edma_backend_init(frontend_callback frontend_init, frontend_callback frontend_cleanup)
{
	int ret;
