...
def get_pcie_ids():
    ...
    if not (os.path.exists(id_file)):
        print_error('Cannot find "cl_id_defines.vh" file for PCIe ID definitions')
...
def get_vivado_version():
    ...
    version_conversion_map = {"v2023.2.2": "v2023.2"}

    for line in tool_str.splitlines():
        if re.search(r"vivado", line, re.IGNORECASE):
            version = line.split()[1]

            if version in version_conversion_map:
...
def main():
    parser = OptionParser()
    parser.add_option("-c", "--cl", dest="cl", help="CL module name", default="cl_dram_hbm_dma")
    ...
    print("==================================================")
    print("Running CL builds")
    print("==================================================")
    for opt, value in options.__dict__.items():
        print(f"{opt:16s} : {str(value):16s}")
    print("==================================================")
    ...
    if options.mode not in ["small_shell"]:
        if options.mode == "xdma_shell":
            print_warning("XDMA shell is not supported at this time. See ERRATA for details.")
    ...
    if os.getenv("CL_DIR") is None:
        print_error("$CL_DIR environment variable not set. Please set by running 'export CL_DIR=<path to CL>'")

    # Check if CL_DIR is in same path as PWD
    if os.path.realpath(os.environ["CL_DIR"]) not in os.path.realpath(os.getcwd()):
        print_error("$CL_DIR path is not in current build dir. Expecting $CL_DIR @ $CWD/../../../<CL_NAME>")
    ...
    # --package-only only records recipes in the tarball manifest, so --aws_clk_gen is not required.
    if not options.package_only and not options.aws_clk_gen and "clock_recipe" in str(sys.argv):
