...
function usage
{
    echo "usage: aws_build_dcp_from_cl.sh [ [-script <vivado_script>] [-clock_recipe_a A0 | A1 | A2] [-clock_recipe_b B0 | B1 | B2 | B3 | B4 | B5] [-clock_recipe_c C0 | C1 | C2 | C3] [-gui] [-foreground] [-notify] [-ignore_memory_requirement] | [-h] | [-H] | [-help] ]"
    echo " "
...
# Set env
export CLOCK_A_RECIPE="$(cut -d'A' -f2 <<<"$clock_recipe_a")"
export CLOCK_B_RECIPE="$(cut -d'B' -f2 <<<"$clock_recipe_b")"
export CLOCK_C_RECIPE="$(cut -d'C' -f2 <<<"$clock_recipe_c")"
