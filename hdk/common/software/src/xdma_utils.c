...
uint32_t
dma_reg_addr(uint32_t target, uint32_t channel, uint32_t offset)
{
  return ((target << 12) | (channel << 8) | offset);
}

void
que_buffer_to_cl(int chan, uint8_t *buf, size_t len)
{
...
void
que_cl_to_buffer(int chan, uint8_t *buf, size_t len)
{
}

void
start_move_to_cl(int chan)
{
  uint64_t base_addr;
  uint32_t data;
  ...
  /* Don't count the first entry. */
  reg_write(base_addr+dma_reg_addr(H2C_SGDMA_TGT, 0, 0x88), h2c_desc_index-1);
  ...
  /* Set pollmode_wb, ie_completed & run bit. */
...
void
start_move_to_buffer(int chan)
{
}

void
is_move_to_cl_done(int chan)
{
}

void
is_move_to_buffer_done(int chan)
{
}
