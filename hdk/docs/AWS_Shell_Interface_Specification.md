...
These signals are not included on the AXI-4 interfaces of the shell.  If connecting to a fabric or component that supports these signals, these values should be used:
...
These parameters are used to control which DDR controllers are implemented in the CL design.  An example instantiation:
...
**There is no performance or frequency difference between the four DRAM controllers regardless whether they resides in the CL or the Shell logic.**
...
1. Amazon’s specific and fixed PCIe VendorID (0x1D0F) and DeviceID.
2. Three BARs:
    * BAR0 - 16KiB
    * BAR2 - 16KiB
    * BAR4 - 4MiB  
3. No BusMaster support.
4. A range of 32-bit addressable registers.
...
1. PCIe BAR0 as a 32-bit non-prefetchable BAR sized as 32MiB.  This BAR maps to the the OCL AXI-Lite interface.
2. PCIe BAR1 as a 32-bit non-prefetchable BAR sized as 2MiB.  This BAR maps to the the BAR1 AXI-Lite interface.
3. PCIe BAR2 as a 64-bit prefetchable BAR sized as 64KiB. This BAR is not CL visible.  This BAR maps to the MSI-X tables and XDMA (if enabled).
4. PCIe BAR4 as a 64-bit prefetchable BAR sized as 128GiB. This BAR may be used to map the entire External/Internal memory space to the instance address space if desired, through `mmap()` type calls or use `fpga_pci_lib` APIs.
5. FLR capability that will reset the CL.
6. BusMaster capability to allow the CL to master transactions towards the instance memory.    
7. CL’s specific PCIe VendorID, DeviceID, VendorSystemID and SubsystemID as registered through `aws ec2 fpgaImageCreate`
...
**_The Developer must NOT assume frequency lock or alignment between clocks from different groups, even if they are set for same frequencies_**  
...
* `cl_sh_flr_done` – Asserted (active-high) for a single clock to acknowledge the FLR. This must be asserted ** within 4 milliseconds ** in response to `sh_cl_flr_assert`. Note due to pipeline delays it is possible `sh_cl_flr_assert` is asserted for some number of clocks after `cl_sh_flr_done`.
...
**_The CL must use Physical Addresses, and developers must be careful not to use userspace/virtual address._**
...
**If a transaction from PCIe is initiated on AXI-Lite (SDA/OCL/BAR1) interfaces with `dw_cnt` greater than 1, i.e. >32bits, the transaction is split into multiple 32 bit transactions by the Shell.**
