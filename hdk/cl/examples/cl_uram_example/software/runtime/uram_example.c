    ...
    if(strcmp("find", command) == 0) {
      ...
      if(strcmp("add", command) == 0) {
        ...
        if(strcmp("del", command) == 0) {
          ...
          printf("Error: Wrong command. Please choose between find, add or del\n");
          return 1;
        }
      }
    }
...
int uram_example(int slot_id, int pf_id, int bar_id, uint32_t value) {
  ...
  while(busy == 1) {
    ...
    if(timeout == 10) {
      ...
      printf("\t\tsudo fpga-clear-local-image -S 0\n");
      printf("And reload your AFI\n");
      printf("\t\tsudo fpga-load-local-image -S 0 -I agfi-xxxxxxxxxxxxxxxxx\n");
    }
  }
