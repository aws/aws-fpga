...
int main(int argc, char **argv) {
  ...
  ret = sde_fill_pkt_data(wr_data_ptr, params.pkt_size * num_descriptors, START_DOUBLE_WORD);
  ...
  while (num_packets < (params.pkt_cnt)) {
    ...
    ret = sde_mgmt_prepare_write(params.slot_id, wr_data_ptr, num_descriptors * params.pkt_size);
    fail_on(ret, cleanup, "Error preparing write");

    // Start the card to host DMA (read) by posting the descriptors for the buffers.
    // The DMA from SDE to host memory will not start until the host memory to SDE
    // DMA starts because the SDE was configured for loopback mode.
    ...
    ret = sde_mgmt_read_data(params.slot_id, rd_data_ptr, num_descriptors * params.pkt_size);
