...
/*
 * This example demonstrates the maximum performance of the host to card SDE.
...
int main(int argc, char **argv) {
  ...
  // with the buffers to be DMA-ed from host memory to the SDE.
  // It will also simultaneously start the DMA from host to card.
  ...
  while (num_packets < (params.pkt_cnt * 1000000)) {
    ...
    num_descriptors = H2C_DESC_COALESCE_CNT;
