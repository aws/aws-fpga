...
#Timestamp passed in from AWS script so that log and output files match
set timestamp [lindex $argv 0]
puts "All reports and intermediate results will be time stamped with $timestamp";

##Identify build strategy for script outputs (file or directory names) - TBD
#if {[string equal $argv0 create_dcp_from_cl.basic.tcl]} {
#    set strategy "basic"
#} elseif {[string equal $argv0 create_dcp_from_cl.default.tcl]} {
#    set strategy "default"
#} elseif {[string equal $argv0 create_dcp_from_cl.explore.tcl]} {
#    set strategy "explore"
#} elseif {[string equal $argv0 create_dcp_from_cl.timing.tcl]} {
#    set strategy "timing"
#} elseif {[string equal $argv0 create_dcp_from_cl.congestion.tcl]} {
...
# ################################################
# Emulate what AWS will do (Bitstream Generation)
# ################################################
...
write_bitstream -force -bin_file -cell CL $CL_DIR/build/${timestamp}_aws_verify_temp_dir/${timestamp}.SH_CL_final.bit
