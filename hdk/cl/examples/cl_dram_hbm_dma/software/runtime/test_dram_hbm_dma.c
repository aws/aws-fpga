...
//#endif
    ...
    // Interrupt mode is temporarily disabled. Refer to ERRATA.md for details.
...
int dma_example(int slot_id, size_t buffer_size) {
    ...
    for (iter = 0; iter < 5; iter++) {
        rc = fpga_dma_burst_write(write_fd, write_buffer, buffer_size,
            iter * MEM_16G);
        fail_on(rc, out, "DMA write failed on iter: %d", iter);
    }
    ...
    for (iter = 0; iter < 5; iter++) {
        rc = fpga_dma_burst_read(read_fd, read_buffer, buffer_size,
            iter * MEM_16G);
        fail_on(rc, out, "DMA read failed on iter: %d", iter);
