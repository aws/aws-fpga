...
The system verilog simulation tests can be run from the [$CL_DIR/verif/scripts](scripts) directory with all supported simulators (HBM is only supported in VCS & QUESTA). You can run tests by calling the make target for that test located in [$CL_DIR/verif/scripts/Makefile.tests](Makefile.tests):
...
# To Run simulations with a 64 GB DDR DIMM (without user-controlled auto-precharge mode)
# NOTE: CL_DRAM_HBM_DMA does not currently have a datapath to all 64 GB of DDR. Only first 32GB is accessible.
make TEST=test_dram_dma USE_64GB_DDR_DIMM=1
