...
//
// deassert_clk_resets()
// Deasssert clock resets in AWS_CLK_GEN block
//
//=============================================================
int deassert_clk_resets(int slot_id) {
    int rc;
    ...
    uint32_t mmcm_lock_expected = 0x151;

    // AWS_CLK_GEN is on PF1-BAR4
    int pf_id  = 1;
    int bar_id = 4;

    // AWS_CLK_GEN specific registers
    uint64_t base_addr  = 0x58000;
    uint64_t grst_reg   = base_addr + 0x10;
    uint64_t sysrst_reg = base_addr + 0x14;
    uint64_t lock_reg   = base_addr + 0x20;

    printf("__INFO__: deassert_clk_resets()\n");
    rc = fpga_pci_attach(slot_id, pf_id, bar_id, fpga_attach_flags, &pci_bar_handle);
    fail_on(rc, out, "Unable to attach to the AFI on slot id %d", slot_id);

    // Clear global reset and sys reset regs
    wdata = 0;
    rc = fpga_pci_poke(pci_bar_handle, grst_reg, wdata);
    fail_on(rc, out, "Failed to write to register @ 0x%08x, value = 0x%08x", (uint32_t) grst_reg, wdata);

    rc = fpga_pci_poke(pci_bar_handle, sysrst_reg, wdata);
    fail_on(rc, out, "Failed to write to register @ 0x%08x, value = 0x%08x", (uint32_t) sysrst_reg, wdata);

    // Now Wait for MMCM lock
    loop_count = 0;
    do {
      rc = fpga_pci_peek(pci_bar_handle, lock_reg, &read_data);
      fail_on(rc, out, "Failed to read from register @0x%08lx", lock_reg);
      loop_count++;
    } while ((read_data != mmcm_lock_expected) && (loop_count < max_loop_count));

    if (loop_count >= max_loop_count) {
      rc = 1;
      fail_on(rc, out, "Timeout: Failed to achieve MMCM lock after %d iterations", loop_count);
    }
