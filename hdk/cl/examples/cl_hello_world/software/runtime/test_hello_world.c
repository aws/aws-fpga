...
/*
 * pci_vendor_id and pci_device_id values below are Amazon's and avaliable to use for a given FPGA
 * slot. Users may replace these with their own if allocated to them by PCI SIG.
 */
...
main(int argc, char **argv)
{
    ...
    /*
     * Initialize the fpga_pci library so we could have access to FPGA PCIe from this
     * applications.
     */
    rc = fpga_pci_init();
    ...
    /*
     * This demo works with single FPGA slot, we pick slot #0 as it works for both
     * f1.2xl and f1.16xl
     */
    slot_id = 0;
    ...
    /*
     * Accessing the CL registers via AppPF BAR0, which maps to sh_cl_ocl_ AXI-Lite bus between
     * AWS FPGA Shell and the CL
     */
 ...
 */
int
peek_poke_example(int slot_id, int pf_id, int bar_id)
{
    int rc;

    /*
     * pci_bar_handle_t is a handler for an address space exposed by one PCI BAR on one of the
     * PCI PFs of the FPGA.
     */
