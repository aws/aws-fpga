...
* AWS Shell DCP is now stored in S3, and fetched/verified when `hdk_setup.sh` script is called.
...
* shell_version, pci_vendor_id, pci_device_id are now become a mandatory parameter in manifest.txt that should be submitted within the tar file
...
Additional tunable auxiliary clocks are generated by the Shell and fed to the CL. The clocks frequencies are set by choosing clock_group_profile from a set of predefined frequencies combination in clock_table[TBD].  

* Clock frequency selection is set while calling `create-fpga-image` is ran, and as the manifest.txt should include the clock_group_profile data.  
* Clock frequency programming in the FPGA slot itself occurs when the AFI is loaded. The list of support frequencies will be released soon. (WIP)
...
* See [AFI_Manifest](./hdk/docs/AFI_Manifest.md) for details on the AFI manifest data. (WIP)
...
** ManagementPF and ApplicationPF are now represented as different PCIe devices in F1 instances:**
...
* CL_HELLO_WORLD and CL_DRAM_DMA examples updated to use these libraries. (WIP)
...
* [hdk_version.txt](./hdk/hdk_version.txt) (NA)
* [shell_version.txt](./hdk/common/shell_current/build/checkpoints/from_aws/shell_version.txt) (NA)
...
### 30.	Simplify handling of unused Shell to CL interfaces (WIP)

* Developers can simply call `include "unused\_**BUS\_NAME**\_template.vh" for every unused interface
* List of potential files to include is available in `$HDK_SHELL_DIR/design/interfaces/unused\*`
* cl_hello_world.sv and cl_dram_dma.sv provides examples (in the each of the file)
