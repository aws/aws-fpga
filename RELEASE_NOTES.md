...
Since F1 announcement at Re:Invent 2016, AWS has been working on the release of the next version of Shell, HDK and SDK.  The upcoming release includes extensive enhancements and capabilities that we believe will benefit developers and answers customers’ requests. AWS has put the upcoming version in the preelease GitHub tree for Preview Customers to get ready for the coming changes.

The preelease GitHub tree is providing F1 Preview developers with documentation and tools to start porting their Custom Logic (CL) designs to work with the upcoming HDK release that will include mandatory Shell changes. Currently, the pre-release tree includes:
1) Detailed documentation of new features and new interfaces 
2) Updated Shell Simulation environment to match the new Shell/CL interface, and 
3) Updated Shell DCP

Providing the Documentation and Shell Simulation allows Developers to start work on porting designs to the 1.1.0 shell version.  The Shell DCP included is for initial CL timing purposes only and it is expected to change prior to the Master branch release. AFI ingestion with this DCP is not supported and will generate an error if used.

## Release 1.1.0 Content
...
All AFIs created with Shell versions prior to 1.1.0 will no longer correctly load on an F1 instance, hence a `load-fpga-image` command executed with an AFI created prior to 1.1.0 will return an error and not load.

The HDK updates include a new AWS Shell, CL examples, and build utilities.  The CL examples provide more detailed examples and the build utilities improve timing closure within the CL.  

The SDK updates include updated FPGA Image Management Tools, drivers for DMA (Elastic DMA, or EDMA), drivers for SDAccel, and updated EC2 CLI commands.  A new JSON file exists for the CLI for generating AFIs and managing AFI permissions.  Updating to this JSON file is recommended, but not required for updating to the new Shell version.  
...
*	Updated the xdc timing constrains under [constraints](./hdk/common/shell_current/build/constraints) to match the new interfaces
*	Updated CL HELLO WORLD example
*	Updated clean_log.pl [scripts](./hdk/common/shell_current/build/scripts/clean_log.pl)


### 2.	New Integrated DMA. AWS Shell now includes DMA capabilities on behalf of the CL
*	The DMA bus toward the CL is multiplexed over sh_cl_pcis AXI interface so the same address space can be accessed via DMA or directly via PCIe AppPF BAR4
*	DMA usage is covered in the new CL_DRAM_DMA example RTL verification/simulation and Software 
*	A corresponding AWS Elastic DMA ([EDMA](./sdk/linux_kernel_drivers/edma)) driver is provided under /sdk/linux_kernel_drivers/edma
...
### 3.	CL / User-defined interrupts.  The CL can now request sending MSI-X to the instance CPU
*	Added new req/ack interface on Shell/CL interface
*	Usage covered in new CL_DRAM_DMA example: RTL verification/simulation and software
...
### 5.	Create-fpga-image `-shell_version` argument is obsolete 
*	Shell_version becomes a mandatory parameter in manifest.md
...
*	All xdc scripts have been updated to clk_main_a0 and to reference a table with the possible clocks’ frequencies combinations [TBD]
...
### 7.	Additional Tunable Auxiliary Clocks
Additional Tunable Auxiliary Clocks are generated by the Shell and fed to the CL. These clocks frequencies are chosen from a set of predefined frequencies combination in clock_tablep[TBD]  This selection is made on CL ingestion when `create-fpga-image` is ran, and target frequencies are set as part of the manifest data.  Clock frequency programming in the FPGA slot itself occurs when the AFI is loaded. The list of support frequencies will be released soon.
...
*	xdc is automatically updated with the target frequency

### 8.	Additional PCIe BARs
...
* ManagementPF added additional PF2 BAR for Virtual JTAG
* PF4 BAR for upcoming SDAccel management. 

*	[AWS_Shell_Interface_Specification](./hdk/docs/AWS_Shell_Interface_Specification.md) covers these changes in detail
*	[AWS_Fpga_Pcie_Memory_Map](./hdk/docs/AWS_Fpga_Pcie_Memory_Map.md) covers the address map in details
*	The fpga_pci libraries in /sdk/userspace provide simple APIs to take advantage of these BARs
...
### 10.	Added wider access on the Shell to CL AXI4 512-bit bus (sh_cl_pcis)
Wider access provides for high bandwidth DMA and host to FPGA access

*	Instance CPU can now burst full 64-byte write burst to AppPF PCIe BAR4 if mapped as Burstable (Aka WC: WriteCombine)
*	Pci_poke_burst() and pci_poke64() C-API where added to fpga_pci library to take advantage of this
*	CL_DRAM_DMA and CL_HELLO_WORLD examples support for wider access was added

### 11.	Support larger than 32-bit access to PCIe space
Large inbound transaction going to AppPF PCIe BAR4 will be passed onward to the CL.  Large inbound transactions going to the other BARs will be split by the Shell to multiple 32-bit accesses.
...
*	FPGA Management Tool [metrics output](./sdk/userspace/fpga_management_tools/README.md) covers the additional error handling

### 13.	Expanded AXI ID space throughout the design
The AXI buses on Shell/CL Interface supports an expanded number of AXI ID bits to allow for bits to be added by AXI fabrics.  See [AWS_Shell_Interface_Specification](./hdk/docs/AWS_Shell_Interface_Specification.md) for more details

### 14.	Shell to CL interface metrics.  
New metrics for monitoring the Shell to CL are available from the AFI Management Tools.  

*	See [fpga_image_tools readme](./sdk/management/fpga_image_tools/README.md) for more details.

### 15.	Virtual LED/DIP Switches.  
Added CL capability to present virtual LEDs and receive virtual DIP switches indications, set and read by FPGA management Tools and without involving CL logic, providing the developer an environment similar to developing on local boards with LED and DIP switches

*	See sdk/user/fpga_image_tools/README.md for description of the API
...
*	Support for generating .ltx files after `create-fpga-image` was added.  .ltx file is required when running interactive ILA/VIO debug
*	Build tcl and xdc includes the required changes to enable Virtual JTAG 
*	See CL_HELLO_WORLD for examples on using Virtual JTAG and XVC for debug. 

### 17.	Examples summary table added under /hdk/cl/examples/cl_examples_tables.md

### 18.	Updated CL_HELLO_WORLD Example

*	Matching the new Shell/CL interface
*	Add support for 32-bit peek/poke via ocl_ AXI-L but
*	Adding Virtual JTAG support with Xilinx ILA and VIO debug cores
...
*	See [CL_DRAM_DMA README](./hdk/cl/example/cl_dram_dma/README.md)
...
The Software Programmer View document is added to explain the various ways a linux user-space application can work with AWS FPGAs

*	See [Programmers_View](./hdk/docs/Programmers_View.md) for more details
...
*	See [FPGA Management Tools](./sdk/userspace/management/README.md) for more details

### 25.	Upgrade to Vivado 2016.04 Build 
*	The FPGA Development AMI will include Vivado 2016.04
*  Older Vivado versions will not be support

### 26.	Embed the HDK version and Shell Version as part of git tree
*	/hdk/hdk_version.txt
*	/hdk/common/shell_current/build/checkpoints/from_aws/shell_version.txt

### 27.	Initial Release of SDAccel and OpenCL Support [Coming soon]
*	Updated documentation in /sdk/SDAccel
*	OpenCL runtime HAL for supporting SDAccel and OpenCL ICD in /sdk/userspace
*	SDAccel build post-processing to register SDAccel xcl.bin as AFI. See TBD
...
* Vivado License needs to support VU9p ES1 FPGA
* Vivado License needs to support encryption
* This release is tested and validated with Vivado 2016.4
...
### How do I know which HDK version I have on my instance/machine?
Look for /hdk/hdk_version.txt.  
### How do I know what my Shell Version is?
...
### How do I know what version of FPGA Image management tools are running on my instance?
...
### When do I need to repeat the AFI ingestion process for my CL design?
...
### Can I use my existing AFI with the new HDK release?
No.  Existing AFIs will not load with the new Shell.  
### How do I update my design with this release?
...
### How do I get support for this release?
...
### Will I need to restart my instance?  
...
### Will there be mixed instances in the fleet with Shell 1.0.0 and 1.1.0?  
...
### How do I know which HDK release I am working with?
