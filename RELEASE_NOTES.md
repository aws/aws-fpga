...
* DMA support is in Beta stage with a known issue for DMA transactions that cross 4K address boundaries.  See [Kernel_Drivers_README](./sdk/linux_kernel_drivers/README.md) for more information on restrictions for this release
...
*The integrated DMA function is in Beta stage.  There is a known issue with DMA addresses crossing 4K page boundaries.  The failure can be triggered by transfers that start on an address other than 4K aligned AND cross the 4K page boundary.  Transfers that do not cross the 4K boundary OR transfers that start at the beginning of a 4K page and greater than 4K size are not susceptible to the error.  Developers should use 4K aligned address boundaries on any transfer that can cross a 4K boundary to avoid the issue.
